...
with st.expander("Developer Audit: Feature Alignment Check", expanded=True):
    ...
    if has_single or has_bulk:
        ...
        # Sort so mismatches always appear in the first rows
        ...
        def highlight_diff(row):
            ...
            return ['background-color: rgba(255, 75, 75, 0.4)' if row["Mismatch"] else '' for _ in row]
        ...
        st.dataframe(comparison.drop(columns=["Mismatch"]).style.apply(highlight_diff, axis=1), use_container_width=True)
