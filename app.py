...
# --- THE ESG-READY PDF ENGINE (v5.9 RESTORED) ---
def create_pdf(df, fig=None):
    ...
    # RESTORED: Strategic Overview Section
    pdf.set_font("helvetica", 'B', 14); pdf.cell(0, 10, "Strategic Overview:", ln=True)
    pdf.set_font("helvetica", '', 11)
    avg_mpg = df['Predicted_MPG'].mean()
    overview_text = f"The current fleet trajectory indicates a healthy high-efficiency core. With a calculated fleet average of {avg_mpg:.1f} MPG, the organization is well-positioned for data-driven optimization."
    pdf.multi_cell(0, 7, overview_text)
    pdf.ln(5)

    # RESTORED: OG KPI Grid
    ...
    # Visual Analytics Page (If Figure Provided)
    if fig:
        pdf.add_page()
    ...
    # RESTORED: Critical Asset Highlights Table
    ...
    # RESTORED: Recommendations Footer
    pdf.set_font("helvetica", 'B', 12); pdf.cell(0, 10, "Recommendations", ln=True)
    pdf.set_font("helvetica", '', 10)
...
def prepare_ai_input(df, scaler_X):
    template = np.zeros((len(df), 12)); input_df = pd.DataFrame(template, columns=RNN_COLS)
...
if mode == "Single Vehicle":
    ...
    if st.button("Generate AI Prediction"):
        ...
        ai_in = np.repeat(prepare_ai_input(cleaned_df, scaler_X)[:, np.newaxis, :], 5, axis=1)
    ...
    if file:
        ...
        if st.button("Process Intelligence"):
            ai_in = np.repeat(prepare_ai_input(df_processed, scaler_X)[:, np.newaxis, :], 5, axis=1)
            raw_preds = np.expm1(scaler_y.inverse_transform(model.predict(ai_in))).flatten()
            final_mpg = [apply_hybrid_reality_logic(p, df_raw.iloc[i].get("Model Year", 2024), df_raw.iloc[i].get("Make", "Unknown"), "Mid-Size", "Regular", 2.0, 4, 200) for i, p in enumerate(raw_preds)]
